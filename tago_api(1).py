...
class TagoClient:
    ...
    # TAGO 세부 경로는 대소문자를 구분합니다.
    # 반드시 공공데이터포털 공식 명세의 철자를 그대로 사용해야 합니다.
    OP_CITY_CODES = "GetCtyCodeList"
    OP_STATIONS_BY_CITY = "GetCtyAcctoTrainSttnList"
    OP_TIMETABLE = "GetStrtpntAlocFndTrainInfo"
    ...
    def _request(
        ...
        if response.status_code == 404:
            raise TagoAPIError(
                "TAGO API 세부 경로를 찾지 못했습니다"
                f"(HTTP 404, 작업: {operation}). "
                "API 작업명의 대소문자와 공식 명세를 확인하세요."
            )
    ...
    def get_city_codes(self) -> list[dict[str, str]]:
        body = self._request(self.OP_CITY_CODES)
